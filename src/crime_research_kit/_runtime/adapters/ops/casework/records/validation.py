...
from crime_research_kit._runtime.core.casefile import RECORD_FILES, case_path, ensure_case, read_jsonl, record_path
...
def _schema_roots() -> list[Path | Traversable]:
    ...
    repo_root = source_repo_root(Path(__file__).resolve())
    if repo_root is not None:
        checkout = repo_root / "docs" / "schemas"
        if checkout.exists():
            roots.append(checkout)
    roots.append(files("crime_research_kit._runtime.core.models").joinpath("schemas_data"))
    return roots
