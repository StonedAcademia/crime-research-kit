...
import { detail, inspector, short, shortId, text } from "./model";
...
function renderNetwork(root: HTMLElement, data: ConsoleData): void {
  root.replaceChildren();
  const rawNodes = (data.data.nodes ?? []).map((row) => ({ data: { id: text(row.node_id), label: short(row.label, 28), row } })).filter((item) => item.data.id);
  const rawNodeIds = new Set(rawNodes.map((item) => item.data.id));
  ...
  if (!nodes.length) {
    canvas.textContent = "No source-supported relationship edges are available for this view.";
    return;
  }
  const cy = cytoscape({
    container: canvas,
    ...
    style: [
      { selector: "node", style: { label: "data(label)", width: 24, height: 24, "background-color": "#fff", "border-color": "#315b77", "border-width": 2, color: "#191817", "font-size": 8, "min-zoomed-font-size": 7, "text-wrap": "wrap", "text-max-width": 84, "text-valign": "bottom", "text-halign": "center", "text-margin-y": 5, "text-background-color": "#ffffff", "text-background-opacity": 0.78, "text-background-padding": 2 } },
      { selector: "edge", style: { width: 1.4, "line-color": "#7b8790", "target-arrow-color": "#7b8790", "target-arrow-shape": "triangle", "curve-style": "bezier", opacity: 0.78 } },
    ],
    layout: { name: "cose", animate: false, fit: true, padding: 96, nodeRepulsion: 14000, idealEdgeLength: 120, edgeElasticity: 80, numIter: 1200 },
  });
  cy.one("layoutstop", () => {
    cy.fit(undefined, 96);
    cy.center();
